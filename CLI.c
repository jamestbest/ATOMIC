...
int main(int argc, char** argv) {
    ...
    compile(ATOM_VR__CLI_ENTRY_POINT, ATOM_VR__CLI_OUTPUT_NAME);
...
bool verify_args(int argc, char** argv) {
    if (argc <= 1) {
        puts(ATOM_CT__CLI_ERR_ARG_NONE);
        return false;
    }

    if (argv[1] == 0) {
        puts(ATOM_CT__CLI_ERR_ARG_NULL);
        return false;
    }
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg[0] == '-') {
            if (arg_len >= 2 && arg[1] == '-') {
                ...
                parse_option(arg, arg_len, argv, argc, &i); //-o optionarg optionarg   e.g. -o tok ast  --this can map to flags.  -outname main.out --changes output name
            }
        } else {
...
void parse_option_entry(charp_vec* args) {
    if (args->pos != 1) printf(ATOM_CT__CLI_WRN_OPT_ARG_COUNT, ATOM_CT__OPTION_E_STR);

    ATOM_VR__CLI_ENTRY_POINT = charp_vec_get(args, 0);
}

void parse_option_output(charp_vec* args) {
    if (args->pos != 1) printf(ATOM_CT__CLI_WRN_OPT_ARG_COUNT, ATOM_CT__OPTION_O_STR);
...
void parse_option_out(charp_vec* args) {
    ...
    for (uint i = 0; i < args->pos; i++) {
        ...
        if (index == -1) {
            printf(ATOM_CT__CLI_WRN_OPT_ARG_INVLD, ATOM_CT__OPTION_OUT_STR, arg);
            continue;
        }
...
void parse_option(char* arg, uint arg_len, char** argv, int argc, int* i) {
    ...
    if (args.pos == 0) {
        printf(ATOM_CT__CLI_ERR_OPT_ARG, arg);
        exit(ARGERR);
    }
...
void parse_flag(char* arg) {
    ...
    if (!ret) {
        printf(ATOM_CT__CLI_WRN_OPT_FLG_INVLD, &arg[start]);
        return;
    }
}